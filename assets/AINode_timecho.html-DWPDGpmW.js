...
<span class="line">+--------------------------------------------+-----------------------------+</span>
<span class="line">Total line number = 48</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="使用count窗口函数的示例" tabindex="-1"><a class="header-anchor" href="#使用count窗口函数的示例"><span>使用count窗口函数的示例</span></a></h4><p>该窗口主要用于计算式任务，当任务对应的模型一次只能处理固定行数据而最终想要的确实多组预测结果时，使用该窗口函数可以使用点数滑动窗口进行连续推理。假设我们现在有一个异常检测模型anomaly_example(input: [24,2], output[1,1])，对每24行数据会生成一个0/1的标签，其使用示例如下：</p><div class="language-Shell line-numbers-mode" data-highlighter="prismjs" data-ext="Shell" data-title="Shell"><pre><code><span class="line">IoTDB&gt; select s1,s2 from root.**</span>
<span class="line">+-----------------------------+-------------------+-------------------+</span>
<span class="line">|                         Time|    root.eg.etth.s0|    root.eg.etth.s1|</span>
<span class="line">+-----------------------------+-------------------+-------------------+</span>
<span class="line">|1990-01-01T00:00:00.000+08:00|             0.7855|              1.611|</span>
...
<span class="line">+-----------+----------+----------+------------+---------+----------+----------+</span>
<span class="line">Total line number = 48</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们将对油温的预测的结果和真实结果进行对比，可以得到以下的图像。</p><p>图中10/24 00:00之前的数据为输入模型的过去数据，10/24 00:00后的蓝色线条为模型给出的油温预测结果，而红色为数据集中实际的油温数据（用于进行对比）。</p><figure><img src="https://alioss.timecho.com/docs/img/AINode-analysis1.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>可以看到，我们使用了过去96个小时（4天）的六个负载信息和对应时间油温的关系，基于之前学习到的序列间相互关系对未来48个小时（2天）的油温这一数据的可能变化进行了建模，可以看到可视化后预测曲线与实际结果在趋势上保持了较高程度的一致性。</p><h3 id="功率预测" tabindex="-1"><a class="header-anchor" href="#功率预测"><span>功率预测</span></a></h3><p>变电站需要对电流、电压、功率等数据进行电力监控，用于检测潜在的电网问题、识别电力系统中的故障、有效管理电网负载以及分析电力系统的性能和趋势等。</p><p>我们利用某变电站中的电流、电压和功率等数据构成了真实场景下的数据集。该数据集包括变电站近四个月时间跨度，每5 - 6s 采集一次的 A相电压、B相电压、C相电压等数据。</p><p>测试集数据内容为<a href="https://alioss.timecho.com/docs/img/data.csv" target="_blank" rel="noopener noreferrer">data</a>。</p><p>在该数据集上，IoTDB-ML的模型推理功能可以通过以往A相电压，B相电压和C相电压的数值和对应时间戳，预测未来一段时间内的C相电压，赋能变电站的监视管理。</p><h4 id="步骤一-数据导入-1" tabindex="-1"><a class="header-anchor" href="#步骤一-数据导入-1"><span>步骤一：数据导入</span></a></h4><p>用户可以使用tools文件夹中的<code>import-csv.sh</code> 导入数据集</p><div class="language-Bash line-numbers-mode" data-highlighter="prismjs" data-ext="Bash" data-title="Bash"><pre><code><span class="line">bash ./import-csv.sh -h 127.0.0.1 -p 6667 -u root -pw root -f ../../data.csv</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><h4 id="步骤二-模型导入-1" tabindex="-1"><a class="header-anchor" href="#步骤二-模型导入-1"><span>步骤二：模型导入</span></a></h4><p>我们可以在iotdb-cli 中选择内置模型或已经注册好的模型用于后续的推理。</p><p>我们采用内置模型STLForecaster进行预测，STLForecaster 是一个基于 statsmodels 库中 STL 实现的时间序列预测方法。</p><h4 id="步骤三-模型推理-1" tabindex="-1"><a class="header-anchor" href="#步骤三-模型推理-1"><span>步骤三：模型推理</span></a></h4><div class="language-Shell line-numbers-mode" data-highlighter="prismjs" data-ext="Shell" data-title="Shell"><pre><code><span class="line">IoTDB&gt; select * from root.eg.voltage limit 96</span>
<span class="line">+-----------------------------+------------------+------------------+------------------+</span>
...
<span class="line">+-------+</span>
<span class="line">Total line number = 144</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们将检测为异常的结果进行绘制，可以得到以下图像。其中蓝色曲线为原时间序列，用红色点特殊标注的时间点为算法检测为异常的时间点。</p><figure><img src="https://alioss.timecho.com/docs/img/s6.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>可以看到，Stray模型对输入序列变化进行了建模，成功检测出出现异常的时间点。</p>`,113))])}const h=i(o,[["render",v],["__file","AINode_timecho.html.vue"]]),b=JSON.parse('{"path":"/zh/UserGuide/V1.3.0-2/User-Manual/AINode_timecho.html","title":"AI能力（AINode）","lang":"zh-CN","frontmatter":{"description":"AI能力（AINode） AINode 是 IoTDB 在ConfigNode、DataNode后提供的第三种内生节点，该节点通过与 IoTDB 集群的 DataNode、ConfigNode 的交互，扩展了对时间序列进行机器学习分析的能力，支持从外部引入已有机器学习模型进行注册，并使用注册的模型在指定时序数据上通过简单 SQL 语句完成时序分析任务的...","head":[["link",{"rel":"alternate","hreflang":"en-us","href":"https://iotdb.apache.org/UserGuide/V1.3.0-2/User-Manual/AINode_timecho.html"}],["meta",{"property":"og:url","content":"https://iotdb.apache.org/zh/UserGuide/V1.3.0-2/User-Manual/AINode_timecho.html"}],["meta",{"property":"og:site_name","content":"IoTDB Website"}],["meta",{"property":"og:title","content":"AI能力（AINode）"}],["meta",{"property":"og:description","content":"AI能力（AINode） AINode 是 IoTDB 在ConfigNode、DataNode后提供的第三种内生节点，该节点通过与 IoTDB 集群的 DataNode、ConfigNode 的交互，扩展了对时间序列进行机器学习分析的能力，支持从外部引入已有机器学习模型进行注册，并使用注册的模型在指定时序数据上通过简单 SQL 语句完成时序分析任务的..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://alioss.timecho.com/docs/img/h1.PNG"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:locale:alternate","content":"en-US"}],["meta",{"property":"og:updated_time","content":"2024-11-11T11:01:16.000Z"}],["meta",{"property":"article:modified_time","content":"2024-11-11T11:01:16.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"AI能力（AINode）\\",\\"image\\":[\\"https://alioss.timecho.com/docs/img/h1.PNG\\",\\"https://alioss.timecho.com/docs/img/AINode-call1.png\\",\\"https://alioss.timecho.com/docs/img/AINode-call2.png\\",\\"https://alioss.timecho.com/docs/img/AINode-call3.png\\",\\"https://alioss.timecho.com/docs/img/AINode-analysis1.png\\",\\"https://alioss.timecho.com/docs/img/AINode-analysis2.png\\",\\"https://alioss.timecho.com/docs/img/s6.png\\"],\\"dateModified\\":\\"2024-11-11T11:01:16.000Z\\",\\"author\\":[]}"]]},"headers":[{"level":2,"title":"优势特点","slug":"优势特点","link":"#优势特点","children":[]},{"level":2,"title":"基本概念","slug":"基本概念","link":"#基本概念","children":[]},{"level":2,"title":"安装部署","slug":"安装部署","link":"#安装部署","children":[]},{"level":2,"title":"使用指导","slug":"使用指导","link":"#使用指导","children":[{"level":3,"title":"注册模型","slug":"注册模型","link":"#注册模型","children":[]},{"level":3,"title":"查看模型","slug":"查看模型","link":"#查看模型","children":[]},{"level":3,"title":"删除模型","slug":"删除模型","link":"#删除模型","children":[]},{"level":3,"title":"使用内置模型推理","slug":"使用内置模型推理","link":"#使用内置模型推理","children":[]},{"level":3,"title":"使用深度学习模型推理","slug":"使用深度学习模型推理","link":"#使用深度学习模型推理","children":[]}]},{"level":2,"title":"权限管理","slug":"权限管理","link":"#权限管理","children":[]},{"level":2,"title":"实际案例","slug":"实际案例","link":"#实际案例","children":[{"level":3,"title":"电力负载预测","slug":"电力负载预测","link":"#电力负载预测","children":[]},{"level":3,"title":"功率预测","slug":"功率预测","link":"#功率预测","children":[]},{"level":3,"title":"异常检测","slug":"异常检测","link":"#异常检测","children":[]}]}],"git":{"createdTime":1702551046000,"updatedTime":1731322876000,"contributors":[{"name":"W1y1r","email":"150988475+W1y1r@users.noreply.github.com","commits":1}]},"readingTime":{"minutes":20.66,"words":6198},"filePathRelative":"zh/UserGuide/V1.3.0-2/User-Manual/AINode_timecho.md","localizedDate":"2023年12月14日","autoDesc":true}');export{h as comp,b as data};
